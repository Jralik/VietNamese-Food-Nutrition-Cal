...
class FoodVolumePipeline:
    ...
    def analyze(
        ...
        # ── Step 4.5: Anchor Metric Depth to ArUco Ground-Truth Distance ──
        final_depth_mm = depth_result.depth_mm.copy() if depth_result.depth_mm is not None else None
        if (
            scale_result.scale_source == "aruco_marker"
            ...
            if d_pred > 0 and z_marker_true > 0:
                # Additive depth offset alignment: shifts global camera distance to z_marker_true
                # while perfectly preserving the metric object height relief (Z_plane - Z_food)
                depth_offset_mm = z_marker_true - d_pred
                final_depth_mm = np.clip(final_depth_mm + depth_offset_mm, 100.0, 5000.0)
                logger.info(
                    f"Depth anchored to ArUco marker: z_true={z_marker_true:.1f}mm, "
                    f"d_pred={d_pred:.1f}mm -> offset={depth_offset_mm:+.1f}mm"
                )
